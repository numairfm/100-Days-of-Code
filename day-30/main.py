...
def search_json(key):
    ...
    try:
        ...
        messagebox.showerror(title="Oops!", message="No passwords have been stored yet!")
    
    else:
        try:
            stored_email = data[key]["email"]
            stored_password = data[key]["password"]
        except KeyError:
            messagebox.showinfo(title="Search Result", message="No details for the website exists.")
        else:
            messagebox.showinfo(title="Search Result", message=f"Email: {stored_email}\nPassword: {stored_password}")
...
# Input
ui_website_input = tk.Entry(width=30)
ui_website_input.grid(row=1, column=1, columnspan=2, sticky="w")
...
ui_secret_input = tk.Entry(width=30)
...
ui_search_button = tk.Button(text="Search", width=10, bg="white", padx=30, pady=5, command=perform_search)
ui_search_button.grid(row=1, column=2, sticky="e")
